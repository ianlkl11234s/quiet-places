...
export const drainOpening={minX:.55,maxX:1.40,minZ:-1.175,maxZ:-.325,roofY:3,frame:.04,slatWidth:.022,braceWidth:.018};
export const drainSlats=Array.from({length:9},(_,i)=>.59+(i+1)*.77/10);
...
export function drainIsOpen(x:number,z:number){
 ...
 return x>.59&&x<1.36&&z>-1.135&&z<-.365&&drainSlats.every(v=>Math.abs(x-v)>.011)&&drainBraces.every(v=>Math.abs(z-v)>.009);
}
...
export function drainRainOrigin(u:number,v:number){
 ...
 const x0=column===0?.59:drainSlats[column-1]+.011,x1=column===9?1.36:drainSlats[column]-.011;
