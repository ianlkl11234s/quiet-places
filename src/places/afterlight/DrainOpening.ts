...
export const drainBraces=Array.from({length:2},(_,i)=>-1.36+(i+1)*1.42/3);
...
export function drainIsOpen(x:number,z:number){
 ...
 return x>.64&&x<1.66&&z>-1.36&&z<.06&&drainSlats.every(v=>Math.abs(x-v)>.011)&&drainBraces.every(v=>Math.abs(z-v)>.009);
}
...
export function drainRainOrigin(u:number,v:number){
 ...
 const z0=row===0?-1.36:drainBraces[row-1]+.009,z1=row===2?.06:drainBraces[row]-.009;
