...
export const drainOpening={minX:.60,maxX:1.70,minZ:-1.13,maxZ:.37,roofY:3,frame:.04,slatWidth:.022,braceWidth:.018};
export const drainSlats=Array.from({length:9},(_,i)=>.64+(i+1)*1.02/10);
export const drainBraces=Array.from({length:2},(_,i)=>-1.09+(i+1)*1.42/3);
...
export function drainIsOpen(x:number,z:number){
 ...
 return x>.64&&x<1.66&&z>-1.09&&z<.33&&drainSlats.every(v=>Math.abs(x-v)>.011)&&drainBraces.every(v=>Math.abs(z-v)>.009);
}
...
export function drainRainOrigin(u:number,v:number){
 ...
 const z0=row===0?-1.09:drainBraces[row-1]+.009,z1=row===2?.33:drainBraces[row]-.009;
