import {drainRainOrigin} from './DrainOpening.ts';
...
export function rainOrigin(seed:number,cycle:number){const origin=drainRainOrigin(hash(seed+cycle*7.13),hash(seed+cycle*13.71+4));if(hash(seed+cycle*19.17+81)<.5)origin.x=-origin.x;return origin;}
...
export function activeRainCount(strength:number,heavy=false){return Math.floor((heavy?480:160)*Math.max(0,Math.min(1,strength)));}
