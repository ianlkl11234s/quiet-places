import {corridor,drainRainOrigin} from './DrainOpening.ts';
...
export const rainDrops=Array.from({length:480},()=>({phase:random(),seed:random()*1000,speed:4+random()*2.5,length:.035+random()*.065,brightness:.35+random()*.65,width:.00055+random()*.00065}));
const hash=(n:number)=>{const v=Math.sin(n*12.9898)*43758.5453;return v-Math.floor(v);};
export function rainOrigin(seed:number,cycle:number){const origin=drainRainOrigin(hash(seed+cycle*7.13),hash(seed+cycle*13.71+4));if(hash(seed+cycle*19.17+81)<.5)origin.x=2*corridor.centerX-origin.x;return origin;}
...
export function activeRainCount(strength:number,heavy=false){return Math.floor((heavy?240:80)*Math.max(0,Math.min(1,strength)));}
