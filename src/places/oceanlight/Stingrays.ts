...
export async function prepareStingrays():Promise<StingrayFactory>{
  ...
  const templateResources=collectModelResources(template);
  let disposed=false;
  const release=()=>{if(disposed)return;disposed=true;disposeModelResources(templateResources,['skeletons','geometries','materials','textures']);};
  ...
  const factory:StingrayFactory=Object.assign((group:THREE.Group,lighting:StingrayLighting)=>{
    ...
    return {
      ...
      dispose(){if(released)return;released=true;school.removeFromParent();rays.forEach(ray=>{ray.mixer.stopAllAction();ray.mixer.uncacheRoot(ray.model);disposeModelResources({skeletons:collectModelResources(ray.model).skeletons},['skeletons']);ray.shadow.geometry.dispose();ray.shadowMaterial.dispose();});skinMaterials.forEach(m=>m.dispose());release();},
    };
  },{dispose:release});
  return factory;
}
