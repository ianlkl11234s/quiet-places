export function createAudioSystem(){
 let context:AudioContext|undefined;let gain:GainNode|undefined;let enabled=false;let generation=0;
 async function toggle(){
  ...
  await context.resume();if(request!==generation)return false;enabled=!enabled;gain!.gain.setTargetAtTime(enabled?.055:0,context.currentTime,.7);return enabled;
 }
 function visibility(hidden:boolean){if(!context)return;if(hidden)void context.suspend();else if(enabled)void context.resume()}
 function stop(){generation++;enabled=false;if(!context)return;gain!.gain.cancelScheduledValues(context.currentTime);gain!.gain.setValueAtTime(0,context.currentTime);void context.suspend();}
 return {toggle,visibility,stop,dispose:()=>{generation++;enabled=false;return context?.close();}};
}
