...
export async function prepareSnowwindow():Promise<PlaceFactory>{
 const reference=await new THREE.TextureLoader().loadAsync('/textures/snowwindow-exterior-wide-v3.png');reference.colorSpace=THREE.SRGBColorSpace;reference.wrapS=reference.wrapT=THREE.ClampToEdgeWrapping;
 const factory:PlaceFactory=(scene,renderer)=>{
  ...
  const glassMaterial=new THREE.MeshPhysicalMaterial({color:'#dce7e9',roughness:.08,metalness:0,transparent:true,opacity:.035,transmission:0,thickness:.015,depthWrite:false,side:THREE.DoubleSide});
  ...
  // Exterior-only photographic plate, close to the glass to avoid enlarging
  // droplets. No baked interior or duplicate frame; fixed camera is intentional.
  const referenceMaterial=new THREE.MeshBasicMaterial({map:reference,color:'#ffffff',toneMapped:false,depthWrite:false});
  const referenceView=new THREE.Mesh(new THREE.PlaneGeometry(10.5,4.2),referenceMaterial);referenceView.name='snowwindow-reference-backing';referenceView.position.set(4.10,2.5,-4.49);referenceView.renderOrder=-90;root.add(referenceView);
  ...
  return {position:[1.4,2.35,-.95],target:[-.1,2.35,-4.82],cameraMode:'fixed-position',yawRange:0,fov:56,exposure:1.02,hasSimulation:false,waterMode:'',
   update(_dt,elapsed,state){
    ...
    // sky/platform framing. The exterior is cropped horizontally without stretching droplets.
    ...
    referenceMaterial.color.copy(light.tint).multiplyScalar(.28+.72*light.visibility);plaster.material.color.set('#c2c1bc').multiplyScalar(.50+.50*light.visibility);
