...
export function createCurtains(daylight={value:1}){
 ...
 const material=new THREE.MeshStandardMaterial({map:weave,color:'#e3e6e7',roughness:.96,metalness:0,side:THREE.DoubleSide,transparent:true,opacity:.94});
 ...
 for(const side of [-1]){
  const geometry=new THREE.PlaneGeometry(.62,4.0,10,28);
  ...
  mesh.position.set(-1.44,2.58,-4.06);group.add(mesh);
