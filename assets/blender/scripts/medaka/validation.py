...
def main() -> None:
 ...
 fish_count=m['fishCount']
 assert (m['duration'],m['fps'],m['frameCount'],m['fishCount'],m['stride'])==(120.,30,3601,26,12)
 values=struct.unpack('<%sf'%(len(raw)//4),raw); assert len(values)==3601*26*12
 ...
 for i in range(fish_count):
  ...
  assert math.dist(values[a:a+3],values[b:b+3])<1e-4
