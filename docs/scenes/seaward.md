...
## 2026-09-09：沿海寫實微調候選（未採用）

使用者要求依附件 `fa048e37-b46b-4865-a309-b2a8d4b7f70d/pasted-text.txt` 做微調且保留退路；改動前乾淨基準 **ef9bb96**。保留相機、建築、開口、扶手、單隻魟魚、既有升空翻圈順序。主要比較時刻17:30；附件作美術需求，不宣稱已物理驗證。

- 海反光：`Materials.ts` 將蜂巢正弦網改為低頻、破碎且不完整的移動光斑，開口深入以 exp(-depth*.48) 快速衰減，亮度有緩慢對齊包絡。仍是程序反射光近似，未從海面求真實光子路徑。
- 地面：多尺度低窪遮罩分 damp／film／puddle，減少連續濕反光面、依積水程度調擾動法線。主要為粗糙混凝土與暗濕區，少量水膜／積水反射。無真幾何 displacement、無流體厚度解算，反射仍非完整場景鏡像。
- 牆面：依出口距離、牆腳高度、接縫建立潮濕、細長滲痕、局部淡鹽霜、小裂紋與極淡綠污染；非均勻隨機污漬。新舊參數都屬美術尺度，不是化學風化模擬。
- 晚光：球面天空提供淡暖地平線至冷上空的漸層；海面天空反射同步輕暖，陰影保留冷灰。保留時刻日照方向，不加橙色調色或 bloom。
- 魟魚：平飞 .545m，比原版增加12cm；抬升高點仍1.1m。低空有極小俯仰／不對稱bank、尾骨連續微彎，影子隨高度擴散變淡。未增加新下腹專用海光 shader；仍由場景出口燈受光。完整真GLB離地取樣繼續通過。
- 草：`Grass.ts` 右側174葉主叢、左側24短葉與8片零散芽，共206；混合新老、彎折與乾尖，局部沉積根部，絕對 elapsed 微風加延遲柔陣風。非特定物種或生長模擬，無下載模型。

Browser 首次試片發現 GLSL 保留字 patch 導致編譯失敗，已改名 seaPatch 並重新載入確認建築正常。17:30新畫面較乾、較暗且波光更克制，屬刻意候選，尚未取得使用者採用。候選截圖 `exports/seaward-review/realism-1730.png`；舊17:30視覺參考為 `daylight-sunset.png`（非同一動作相位，不能用作精確像素比較）。若拒絕本候選，以其 commit 做 git revert 即回到 ef9bb96 的 tracked 狀態；不要 reset/clean。未合併或發布。

候選收尾：80/80 tests、check:project、build、diff whitespace 通過。build保留既有大chunk提示；browser確認shader修正後建築與材質正常顯示。尚未量測實機效能，亦未聲稱照片級或完整物理驗收。
