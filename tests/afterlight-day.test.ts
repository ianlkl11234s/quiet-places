...
test('Afterlight traverses dawn, high noon, dusk and moon across the full wrapped day',()=>{
 ...
 assert.ok(dawn.incoming[0]>.8&&dusk.incoming[0]<-.8);
 ...
 for(let minute=1;minute<=1440;minute++){
  ...
  if(Math.abs(now.incoming[0]-last.incoming[0])>1)assert.ok(now.sunStrength+now.moonStrength===0&&last.sunStrength+last.moonStrength===0);
  last=now;
 }
});
