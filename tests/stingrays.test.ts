...
test('two skeletal rays animate independently, freeze and release shared model resources once',async()=>{
  ...
  try{
    ...
    const volume=new THREE.Data3DTexture(new Float32Array(8),2,2,2),floor=new THREE.Texture();
    let geometryDisposals=0,materialDisposals=0,volumeDisposals=0,floorDisposals=0;
    ...
    floor.addEventListener('dispose',()=>floorDisposals++);
    const rays=factory(group,{sun:{value:new THREE.Vector3(0,-1,1).normalize()},time:{value:0},level:{value:1.75},strength:{value:5},tint:{value:new THREE.Color('white')},volume,floor});
    rays.update(17);
    ...
    assert.equal(group.children.length,0);assert.equal(geometryDisposals,1);assert.equal(materialDisposals,1);assert.equal(volumeDisposals,0);assert.equal(floorDisposals,0,'borrowed photon floor texture remains owned by its map');
    volume.dispose();floor.dispose();
